...
import { Box, Button, CircularProgress, Grid, IconButton, Rating, TextField, Tooltip, tooltipClasses, Zoom } from "@mui/material";
import { useFormik } from "formik"
import * as Yup from "yup"
import type { AddReviewRequest } from "../../../types/review";
import store, { useAppDispatch, useAppSelector } from "../../../redux/store";
import { addReview } from "../../../redux/slice/customer/reviewSlice";
...
const AddReview = ({ onClose, productId }: AddReviewFormProps) => {
    ...
    const formik = useFormik<AddReviewRequest>({
        ...
        onSubmit: async (values) => {
            ...
            const result = await dispatch(addReview({ request: values, productId: productId }));
            ...
            } else if (addReview.rejected.match(result)) {
                setSnackbar({
                    open: true,
                    message: result.payload || "Unable to submit review. Please try again later!",
                    severity: "error",
                });
            }
        },
    ...
    return (
        <>
            <Box sx={{ maxHeight: "auto" }}>
                <p className="text-xl font-bold text-primary-color text-start relative pb-5">Write Review</p>
